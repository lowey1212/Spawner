...
// Helper to locate a scene component named "SpawnPoint" on a marker actor.
static USceneComponent *FindSpawnPointComponent(AActor *Marker) {
  if (!Marker) {
    return nullptr;
  }
  // First try the default subobject name for efficiency.
  if (USceneComponent *Default =
          Cast<USceneComponent>(Marker->GetDefaultSubobjectByName(TEXT("SpawnPoint")))) {
    return Default;
  }
  // Fallback: search all scene components for one named "SpawnPoint".
  TArray<USceneComponent *> Components;
  Marker->GetComponents<USceneComponent>(Components);
  for (USceneComponent *Comp : Components) {
    if (Comp && Comp->GetName() == TEXT("SpawnPoint")) {
      return Comp;
    }
  }
  return nullptr;
...
void ADAISpawnManager::EditorOnConstruction() {
  // Persist cached marker transforms when saving the level.
  Modify(true);
  ...
  for (FSpawnEntry &Entry : SpawnEntries) {
    if (Entry.bUseMarker && IsValid(Entry.MarkerActor)) {
      ...
      if (USceneComponent *SpawnPoint =
              FindSpawnPointComponent(Entry.MarkerActor)) {
        Entry.CachedSpawnPointTransform = SpawnPoint->GetComponentTransform();
      } else {
        Entry.CachedSpawnPointTransform =
            Entry.MarkerActor->GetActorTransform();
      }
      Entry.bCachedTransformsValid = true;
    } else if (Entry.bUseMarker &&
               !Entry.CachedMarkerTransform.Equals(FTransform::Identity)) {
      // If the marker actor is missing but cached data exists, treat it as
      // valid so spawns can still occur in game builds.
      Entry.bCachedTransformsValid = true;
    }
  }
  ...
  if (UWorld *W2 = GetWorld()) {
    for (const FSpawnEntry &Entry : SpawnEntries) {
      ...
      if (USceneComponent *SpawnPoint =
              FindSpawnPointComponent(Entry.MarkerActor)) {
        MeshLoc = SpawnPoint->GetComponentLocation() + Entry.MeshOffset;
      }
      ...
      if (Entry.ActorClass != nullptr) {
        ...
        Params.bNoFail = true;
        if (AActor *Preview = W2->SpawnActor<AActor>(
                Entry.ActorClass, ActorLoc, FRotator::ZeroRotator, Params)) {
    ...
    for (int32 Attempt = 0; Attempt < MaxAttempts && !bLocationValid;
      ...
      if (bUsingMarker) {
        if (bHasCached) {
          ...
          if (USceneComponent *Comp = FindSpawnPointComponent(Entry.MarkerActor)) {
            MeshLocation = Comp->GetComponentLocation() + Entry.MeshOffset;
          }
        }
#else
        else if (bMarkerValid) {
