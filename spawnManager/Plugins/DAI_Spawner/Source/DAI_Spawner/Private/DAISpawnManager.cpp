...
void ADAISpawnManager::OnConstruction(const FTransform &Transform) {
  ...
  for (FSpawnEntry &Entry : SpawnEntries) {
    if (Entry.bUseMarker && IsValid(Entry.MarkerActor)) {
      ...
      // If the marker actor is missing but we have cached data, treat it as
      // valid.
      Entry.bCachedTransformsValid = true;
    }
  }
...
void ADAISpawnManager::SpawnActorsInternal() {
  ...
  // weights
  // Build a list of candidate entries and their weights. We only ever spawn
  // a single actor so we don't need to track per-entry counts.
  ...
  // Compute the total weight of all candidates
  ...
  // Choose a candidate based on weight
  ...
  // Roll spawn chance once for the selected entry
