...
// DrawDebugArea and ClearNonPersistentDebug are implemented in the editor
// module to avoid compiling editor-only functionality into runtime builds.
    ...
    for (int32 Attempt = 0; Attempt < MaxAttempts && !bLocationValid;
      ...
      if (bUsingMarker) {
        if (bHasCached) {
          ...
          // optional component named "SpawnPoint" if present on the marker.
          ActorLocation =
              Entry.MarkerActor->GetActorLocation() + Entry.ActorOffset;
          MeshLocation = ActorLocation + Entry.MeshOffset;
          if (USceneComponent *Comp =
                  Entry.MarkerActor->FindComponentByClass<USceneComponent>()) {
            if (Comp->GetFName() == TEXT("SpawnPoint")) {
              MeshLocation =
                  Comp->GetComponentLocation() + Entry.MeshOffset;
            }
          }
        }
#else
