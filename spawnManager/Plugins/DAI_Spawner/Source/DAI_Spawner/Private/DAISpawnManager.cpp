...
void ADAISpawnManager::OnConstruction(const FTransform& Transform) {
    ...
    for (UPrimitiveComponent* Comp : EditorPreviewMeshes) {
        if (Comp) {
    ...
    if (UWorld* W2 = GetWorld()) {
        for (const FSpawnEntry& Entry : SpawnEntries) {
            ...
            if (Entry.StaticMesh) {
                ...
                SM->SetHiddenInGame(true);
                SM->bIsEditorOnly = true;
                SM->RegisterComponent();
                SM->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepWorldTransform);
    ...
    if (bDebug && !HasAnyFlags(RF_ClassDefaultObject) && SpawnEntries.Num() > 0) {
        const FSpawnEntry& FirstEntry = SpawnEntries[0];
        FVector BaseLocation = GetActorLocation();

        if (FirstEntry.ActorClass) {
            AActor* TemplateActor = FirstEntry.ActorClass->GetDefaultObject<AActor>();
            if (TemplateActor) {
                TArray<USkeletalMeshComponent*> SkelMeshComps;
                TemplateActor->GetComponents(SkelMeshComps);
                if (SkelMeshComps.Num() > 0 && SkelMeshComps[0]->GetSkeletalMeshAsset()) {
                    USkeletalMeshComponent* PreviewSkel = NewObject<USkeletalMeshComponent>(this);
                    PreviewSkel->SetSkeletalMeshAsset(SkelMeshComps[0]->GetSkeletalMeshAsset());
                    PreviewSkel->SetWorldLocation(BaseLocation);
                    PreviewSkel->SetWorldScale3D(SkelMeshComps[0]->GetComponentScale());
                    PreviewSkel->SetRelativeRotation(SkelMeshComps[0]->GetRelativeRotation());
                    PreviewSkel->SetVisibility(true);
                    PreviewSkel->SetHiddenInGame(true);
                    PreviewSkel->SetMobility(EComponentMobility::Movable);
                    PreviewSkel->bIsEditorOnly = true;
                    PreviewSkel->RegisterComponent();
                    PreviewSkel->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepWorldTransform);
                    EditorPreviewMeshes.Add(PreviewSkel);
                }
            }
        }

        // Add cylinder with red material
        UStaticMesh* CylinderMesh = LoadObject<UStaticMesh>(nullptr, TEXT("/Engine/BasicShapes/Cylinder.Cylinder"));
        if (CylinderMesh) {
            UStaticMeshComponent* Cylinder = NewObject<UStaticMeshComponent>(this);
            Cylinder->SetStaticMesh(CylinderMesh);
            Cylinder->SetWorldLocation(BaseLocation - FVector(0, 0, 10));
            Cylinder->SetWorldScale3D(FVector(1.0f, 1.0f, 0.2f));
            ...
            if (RedMaterial) {
                Cylinder->SetMaterial(0, RedMaterial);
            }

            Cylinder->RegisterComponent();
            Cylinder->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepWorldTransform);
            EditorPreviewMeshes.Add(Cylinder);
        }
    }
#endif
}
...
void ADAISpawnManager::SpawnActors() { SpawnActorsInternal(); }

void ADAISpawnManager::SpawnActorsInternal() {
    if (bWaitForLevelToLoad && RequiredLevelName != NAME_None) {
        ULevelStreaming* Streamed =
            UGameplayStatics::GetStreamingLevel(this, RequiredLevelName);
    ...
    for (int32 i = 0; i < SpawnEntries.Num(); ++i) {
        ...
        if (Entry.bUniqueInstance) {
            if (const TWeakObjectPtr<AActor>* ExistingPtr =
                ActiveInstances.Find(Entry.ActorClass)) {
        ...
        if (Entry.MaxActive > 0) {
            TArray<TWeakObjectPtr<AActor>>& Arr =
                ActiveByClass.FindOrAdd(Entry.ActorClass);
            Arr.RemoveAll(
    ...
    // timer is cleared before setting a new one to avoid multiple timers
    // accumulating if SpawnActors() is called manually when bLoopSpawning is
    // true.
    if (bLoopSpawning) {
...
bool ADAISpawnManager::CanSpawnForEntry_Implementation(
...
void ADAISpawnManager::DespawnAll() {
    ...
    for (TPair<TSubclassOf<AActor>, TWeakObjectPtr<AActor>>& Pair :
        ActiveInstances) {
...
// Retrieve or create a Hierarchical Instanced Static Mesh component for the
// given mesh. When spawning static meshes we reuse existing HISM components to
// improve performance.  If no component exists for a particular mesh one will
// be created, attached to the scene root and registered.
UHierarchicalInstancedStaticMeshComponent*
ADAISpawnManager::GetOrCreateHISM(UStaticMesh* Mesh) {
    if (!Mesh) {
        return nullptr;
    }
    if (UHierarchicalInstancedStaticMeshComponent** Found =
        MeshToHISM.Find(Mesh)) {
        if (*Found) {
            return *Found;
        }
    }
...
// Draw debug preview of the spawn area. This method draws simple shapes to
// visualise the configured spawn volume. For a square area a box is drawn; for
// circle, curve and noise shapes a circle is drawn representing the maximum
// radius.  Debug drawings are non‑persistent.
#if WITH_EDITOR
void ADAISpawnManager::DrawDebugArea() const {
    ...
    default: {
        ...
        DrawDebugCircle(World, Origin, Radius, Segments, Colour, false, Lifetime, 0,
            Thickness, FVector(1, 0, 0), FVector(0, 1, 0), false);
        break;
    }
    }
}
...
// Remove non‑persistent debug visuals.  This flushes persistent debug lines and
// clears all instanced meshes from HISM components so that preview meshes do
// not accumulate.
#if WITH_EDITOR
...
// Tick function. Processes pending spawns and handles debug drawing.  This
// method exists in all builds to ensure the linker resolves the symbol even
// when Tick is only declared in editor builds. Debug drawing is compiled only
// for editor builds.
void ADAISpawnManager::Tick(float DeltaSeconds) {
    ...
    while (PendingSpawns.Num() > 0 && SpawnedThisTick < MaxPerTick) {
        ...
        if (!Item.ActorClass || Item.Count <= 0 ||
            !SpawnEntries.IsValidIndex(Item.PerEntryIndex)) {
        ...
        const bool bUsingMarker = Entry.bUseMarker && (bMarkerValid || bHasCached);
