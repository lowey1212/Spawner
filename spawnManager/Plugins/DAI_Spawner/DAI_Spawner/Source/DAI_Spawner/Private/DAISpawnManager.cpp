  ...
  void ADAISpawnManager::Tick(float DeltaSeconds) {
    ...
    while (PendingSpawns.Num() > 0 && SpawnedThisTick < MaxPerTick) {
      ...
      for (int32 Attempt = 0; Attempt < MaxAttempts && !bLocationValid;
        ...
        if (bUsingMarker) {
          if (bHasCached) {
            ...
            if (const ADAISpawnMarker *Marker =
                    Cast<ADAISpawnMarker>(Entry.MarkerActor)) {
              if (Marker->SpawnPoint) {
                MeshLocation = Marker->SpawnPoint->GetComponentLocation() +
                               Entry.MeshOffset;
              } else {
                MeshLocation =
        ...
        if (GroundHit.bBlockingHit) {
          ...
          if (bAlignToGround) {
            if (bFaceMarkerForward && Entry.bUseMarker &&
              ...
              SpawnRot =
                  UKismetMathLibrary::MakeRotFromZ(GroundHit.ImpactNormal);
            }
          }
        }
        ...
        if (SafePlacementRadius > 0.f) {
          ...
          if (World->OverlapBlockingTestByChannel(
                  ActorLocation, FQuat::Identity, ECC_WorldStatic, Sphere) ||
              World->OverlapBlockingTestByChannel(
                  ActorLocation, FQuat::Identity, ECC_WorldDynamic, Sphere)) {
            continue;
          }
        }
      ...
      AActor *NewActor = World->SpawnActor<AActor>(
          Item.ActorClass, ActorLocation, SpawnRot, Params);
      if (NewActor) {
